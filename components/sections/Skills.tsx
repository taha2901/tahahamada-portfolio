  ...
  return (
    <section id="skills" className="relative py-20 md:py-28 overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        {/* Skills Grid (balanced, not messy) */}
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 sm:gap-4">
          {skills.map((skill, i) => (
            <motion.div
              ...
              className="group"
            >
              <div className="relative h-full rounded-xl border border-white/10 bg-white/5 dark:bg-white/5 backdrop-blur-md px-3 py-3 sm:px-4 sm:py-3 flex flex-col items-center justify-center gap-2 text-center transition-all duration-300 hover:border-brand-400/40 hover:bg-white/10">
                {/* Icon */}
                ...
                {/* Name */}
                <p className="text-xs sm:text-sm font-medium text-slate-800 dark:text-white leading-tight">
                  {isAr ? skill.nameAr : skill.name}
                </p>

                {/* Optional subtle level indicator */}
