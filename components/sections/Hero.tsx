...
export default function Hero() {
  ...
  return (
    <section
      id="home"
      dir={isAr ? "rtl" : "ltr"}
      className="relative bg-white dark:bg-slate-950 min-h-screen lg:h-screen overflow-hidden w-full"
    >
      {/* noise */}
      <div
        className="pointer-events-none absolute inset-0 opacity-[0.03]"
      ...
      {/* vignette */}
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-b from-white dark:from-slate-950 via-white/95 dark:via-slate-950/95 to-white dark:to-slate-950" />
      ...
      <div className="relative z-10 flex flex-col items-center px-5 pt-16 pb-24 sm:px-8 lg:hidden">
        ...
        >
          <motion.span
            {...fadeUp(0.1)}
            className="mt-8 inline-flex items-center gap-2 rounded-full border border-slate-200 dark:border-slate-700/80 bg-slate-100 dark:bg-slate-900/80 px-4 py-1.5 font-mono text-[0.65rem] uppercase tracking-[0.22em] text-slate-600 dark:text-slate-300 backdrop-blur-sm"
          >
            <span className="relative flex h-2 w-2">
              <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-emerald-400 opacity-60" />
              <span className="relative h-2 w-2 rounded-full bg-emerald-400 shadow-[0_0_10px_rgba(52,211,153,.8)]" />
          ...
          <div
            className="relative rounded-full p-[3px] mt-10"
            style={{ background: "linear-gradient(140deg,#67e8f9,#38bdf8,#818cf8,#67e8f9)" }}
          >
            <div className="rounded-full bg-white dark:bg-slate-950 p-[3px]">
              <img
                src="/images/IMG_1693.jpg"
                alt={isAr ? personalInfo.nameAr : personalInfo.name}
                className="h-36 w-36 rounded-full object-cover sm:h-44 sm:w-44"
        ...
        <motion.h1
          {...fadeUp(0.16)}
          className="-mt-10 mb-5 text-center font-display font-extrabold leading-tight tracking-tight text-slate-900 dark:text-white"
          style={{ fontSize: "clamp(1.9rem, 7vw, 2.8rem)" }}
        >
          {isAr ? personalInfo.nameAr : personalInfo.name}
        </motion.h1>
        ...
        <motion.p
          {...fadeUp(0.26)}
          className="mb-5 max-w-sm text-center text-sm leading-relaxed text-slate-500 dark:text-slate-400"
        >
          {isAr ? personalInfo.taglineAr : personalInfo.tagline}
        </motion.p>

        {/* tech pills */}
        <motion.div {...fadeUp(0.3)} className="mb-6 flex flex-wrap justify-center gap-2">
          {techStack.map((tech) => (
            <span
              key={tech}
              className="rounded-full border border-cyan-500/20 bg-cyan-500/[.07] px-3 py-1 text-[0.6rem] font-semibold uppercase tracking-[0.16em] text-cyan-600 dark:text-cyan-300/80"
            >
              {tech}
            </span>
          ))}
        ...
        >
          ...
          <a
            href={`mailto:${personalInfo.email}`}
            className="inline-flex items-center justify-center gap-2 rounded-2xl border border-slate-300 dark:border-slate-600/60 bg-slate-100 dark:bg-slate-900/60 px-7 py-3.5 text-sm font-semibold text-slate-700 dark:text-slate-200 backdrop-blur-sm transition-all hover:border-cyan-400/40 hover:bg-slate-200 dark:hover:bg-slate-800/70 hover:text-slate-900 dark:hover:text-white active:scale-[.97]"
          >
            <MessageCircle size={15} />
            {isAr ? "تواصل الآن" : "Contact Me"}
          </a>
        ...
        <motion.div {...fadeUp(0.38)} className="mb-10 flex items-center gap-2">
          {socialLinks.map(({ href, icon: Icon, label }) => (
            <a
              ...
              rel="noopener noreferrer"
              aria-label={label}
              className="flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 dark:border-slate-700/70 bg-slate-100 dark:bg-slate-900/60 text-slate-500 dark:text-slate-400 transition-all hover:border-cyan-400/40 hover:text-cyan-500 dark:hover:text-cyan-300"
            >
              <Icon size={17} />
            </a>
          ))}
          <div className="mx-2 h-px w-8 bg-slate-300 dark:bg-slate-700/60" />
          <span className="text-[0.62rem] uppercase tracking-widest text-slate-400 dark:text-slate-600">
            {isAr ? "تابعني" : "Follow me"}
          </span>
        </motion.div>

        {/* stats */}
        <motion.div {...fadeUp(0.42)} className="grid w-full max-w-xs grid-cols-2 gap-3">
          {stats.map((s) => (
            <div
              key={s.value}
              className="flex flex-col items-center rounded-2xl border border-slate-200 dark:border-slate-700/50 bg-slate-100 dark:bg-slate-900/80 py-5"
            >
              <span
                className="text-[2rem] font-extrabold leading-none"
                style={{
      ...
      {/* ── DESKTOP LAYOUT ── */}
      <div className="relative z-10 hidden h-full w-full lg:flex lg:items-center lg:justify-between lg:px-16 xl:px-24">
        {/* text column */}
        <div className="flex w-1/2 flex-col items-start gap-5 text-start xl:w-[52%]">
          <motion.span
            {...fadeUp(0.08)}
            className="inline-flex items-center gap-2 rounded-full border border-slate-200 dark:border-slate-700/80 bg-slate-100 dark:bg-slate-900/80 px-4 py-1.5 font-mono text-[0.65rem] uppercase tracking-[0.22em] text-slate-600 dark:text-slate-300 backdrop-blur-sm"
          >
            <span className="relative flex h-2 w-2">
              <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-emerald-400 opacity-60" />
              <span className="relative h-2 w-2 rounded-full bg-emerald-400 shadow-[0_0_10px_rgba(52,211,153,.8)]" />
          ...
          <motion.h1
            {...fadeUp(0.15)}
            className="font-display font-extrabold leading-[1.05] tracking-tight text-slate-900 dark:text-white"
            style={{ fontSize: "clamp(2.2rem, 4vw, 4rem)" }}
          >
            {isAr ? personalInfo.nameAr : personalInfo.name}
          </motion.h1>
          ...
          <motion.p
            {...fadeUp(0.25)}
            className="max-w-md text-sm leading-relaxed text-slate-500 dark:text-slate-400 sm:text-[0.95rem]"
          >
            {isAr ? personalInfo.taglineAr : personalInfo.tagline}
          </motion.p>
          ...
          <motion.div {...fadeUp(0.3)} className="flex flex-wrap gap-2">
            {techStack.map((tech) => (
              <span
                key={tech}
                className="rounded-full border border-cyan-500/20 bg-cyan-500/[.07] px-3 py-1 text-[0.6rem] font-semibold uppercase tracking-[0.16em] text-cyan-600 dark:text-cyan-300/80 transition-colors hover:border-cyan-400/40 hover:text-cyan-500 dark:hover:text-cyan-300"
              >
                {tech}
              </span>
            ))}
          ...
          <motion.div {...fadeUp(0.35)} className="flex items-center gap-3">
            ...
            <a
              href={`mailto:${personalInfo.email}`}
              className="inline-flex items-center gap-2 rounded-2xl border border-slate-300 dark:border-slate-600/60 bg-slate-100 dark:bg-slate-900/60 px-7 py-3.5 text-sm font-semibold text-slate-700 dark:text-slate-200 backdrop-blur-sm transition-all hover:border-cyan-400/40 hover:bg-slate-200 dark:hover:bg-slate-800/70 hover:text-slate-900 dark:hover:text-white active:scale-[.97]"
            >
              <MessageCircle size={15} />
              {isAr ? "تواصل الآن" : "Contact Me"}
            </a>
          ...
          <motion.div {...fadeUp(0.4)} className="flex items-center gap-2">
            {socialLinks.map(({ href, icon: Icon, label }) => (
              <a
                ...
                rel="noopener noreferrer"
                aria-label={label}
                className="flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 dark:border-slate-700/70 bg-slate-100 dark:bg-slate-900/60 text-slate-500 dark:text-slate-400 backdrop-blur-sm transition-all hover:border-cyan-400/40 hover:bg-slate-200 dark:hover:bg-slate-800 hover:text-cyan-500 dark:hover:text-cyan-300"
              >
                <Icon size={17} />
              </a>
            ))}
            <div className="mx-2 h-px w-10 bg-slate-300 dark:bg-slate-700/60" />
            <span className="text-[0.65rem] uppercase tracking-widest text-slate-400 dark:text-slate-600">
              {isAr ? "تابعني" : "Follow me"}
            </span>
          </motion.div>
        </div>
        ...
        >
          <div className="relative w-[280px] xl:w-[310px]">
            ...
            <div
              className="relative rounded-[1.75rem] p-[1px]"
              style={{
                background: "linear-gradient(160deg,rgba(56,189,248,.4),rgba(129,140,248,.22),rgba(255,255,255,.04))",
              }}
            >
              <div className="relative overflow-hidden rounded-[1.72rem] bg-white/90 dark:bg-slate-950/90 p-4 backdrop-blur-xl">
                <div className="relative overflow-hidden rounded-[1.4rem] border border-black/[.07] dark:border-white/[.07]">
                  <img
                    src="/images/IMG_1693.jpg"
                    alt={isAr ? personalInfo.nameAr : personalInfo.name}
                    className="h-[280px] w-full object-cover xl:h-[320px]"
                ...
                <div className="mt-4 grid grid-cols-2 gap-3">
                  {stats.map((s) => (
                    <div
                      key={s.value}
                      className="flex flex-col items-center rounded-2xl border border-slate-200 dark:border-slate-700/50 bg-slate-100 dark:bg-slate-900/80 py-4"
                    >
                      <span
                        className="text-[1.8rem] font-extrabold leading-none"
                        style={{
